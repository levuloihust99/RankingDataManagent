...
export const SampleEditor = ({ visible = true }) => {
    ...
    const handleCallCortex = (e) => {
        ...
        callCortex({ text: prompt }).then(async (resp) => {
            if (resp.status === 200) {
                alertDispatch({
                    type: "ADD_MESSAGE",
                    item: {
                        type: "success",
                        message: "Called cortex successfully",
                    },
                })
                const completion = await resp.text()
                datasetDispatch({
                    type: "ADD_OUTPUT_WITH_CONTENT",
                    content: completion,
                    sampleId,
                })
            } else {
